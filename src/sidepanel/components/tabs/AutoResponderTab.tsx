...
export const AutoResponderTab: React.FC = () => {
  ...
  const handleAddQuickTemplate = async (type: 'vendas' | 'agendamento' | 'preco') => {
    if (type === 'vendas') {
      await autoResponderRepository.addRule({
        title: 'Apresentação Comercial & Serviços',
        triggerKeywords: ['como funciona', 'o que voces fazem', 'apresentacao', 'sobre', 'servicos', 'serviços'],
        matchType: 'contains',
        responseTemplate: 'Olá! Somos especializados em captação de clientes e vendas no WhatsApp. Ajudamos sua empresa a crescer com automação e IA. Gostaria de ver como funciona?',
        useAi: true,
        aiPrompt: 'Apresente nossa empresa com entusiasmo e convide o lead para conhecer mais.',
        active: true
      });
    } else if (type === 'agendamento') {
      await autoResponderRepository.addRule({
        title: 'Agendamento de Reunião',
        triggerKeywords: ['reuniao', 'reunião', 'agendar', 'horario', 'horário', 'conversar', 'call'],
        matchType: 'contains',
        responseTemplate: 'Perfeito! Qual seria o melhor dia e horário para batermos um papo rápido de 10 minutos?',
        useAi: true,
        aiPrompt: 'Responda com cordialidade e solicite qual o melhor dia e horário para uma rápida conversa.',
        active: true
      });
    } else {
      await autoResponderRepository.addRule({
        title: 'Valores & Orçamento',
        triggerKeywords: ['preco', 'preço', 'valor', 'quanto custa', 'tabela', 'orcamento', 'orçamento'],
        matchType: 'contains',
        responseTemplate: 'Temos planos sob medida com excelente custo-benefício. Posso te apresentar os detalhes agora?',
        useAi: true,
        aiPrompt: 'Informe que temos valores acessíveis e convide para enviar a proposta.',
        active: true
      });
    }
  };
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-slate-950 p-6 space-y-6 max-w-6xl mx-auto">
      {/* 1. HERO MASTER CARD */}
      <div className="bg-gradient-to-r from-slate-900 via-slate-900/90 to-slate-900 border border-slate-800 rounded-2xl p-5 shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-start gap-3.5">
          <div
            className={`w-12 h-12 rounded-2xl flex items-center justify-center text-white shadow-lg transition-all shrink-0 ${
              isActive
                ? 'bg-gradient-to-tr from-emerald-600 to-teal-400 shadow-emerald-500/30 animate-pulse'
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-bold text-slate-100">Robô de Atendimento WhatsApp</h2>
              <span
              ...
              >
                {isActive ? '🟢 Ativo Respondendo' : '⏸️ Pausado'}
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-1 max-w-xl leading-relaxed">
              Responde automaticamente as pessoas que te mandam mensagem no WhatsApp, tira dúvidas e qualifica clientes 24 horas por dia.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2.5 self-end md:self-auto shrink-0">
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-extrabold transition-all shadow-lg active:scale-95 cursor-pointer ${
              isActive
          ...
          >
            ...
            <span>{isActive ? '🟢 Robô Ligado (Clique para pausar)' : '▶️ Ligar Robô no WhatsApp Agora'}</span>
          </button>
        </div>
      </div>

      {/* 1.1 MODELOS PRONTOS EM 1 CLIQUE */}
      <div className="bg-slate-900/40 border border-slate-800 rounded-2xl p-4 space-y-2.5">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-bold text-slate-200">
            <Sparkles className="w-4 h-4 text-emerald-400" />
            <span>Adicionar Respostas Prontas em 1 Clique (Recomendado para Começar):</span>
          </div>
          <button
          ...
          >
            ...
            <span>Restaurar Todas as Respostas Recomendadas</span>
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <button
            type="button"
            onClick={() => handleAddQuickTemplate('vendas')}
            className="p-3 bg-slate-950/80 hover:bg-slate-900 border border-slate-800 hover:border-emerald-500/40 rounded-xl text-left transition shadow-sm group"
          >
            ...
            <p className="text-[10px] text-slate-400 mt-1 leading-snug">
              Apresenta a empresa e convida o lead para conversar.
            </p>
          </button>

          <button
            type="button"
            onClick={() => handleAddQuickTemplate('agendamento')}
            className="p-3 bg-slate-950/80 hover:bg-slate-900 border border-slate-800 hover:border-teal-500/40 rounded-xl text-left transition shadow-sm group"
          >
            <div className="text-xs font-bold text-teal-400 group-hover:text-teal-300 flex items-center justify-between">
              <span>📅 Agendar Reunião / Horário</span>
              <Plus className="w-3.5 h-3.5 opacity-60 group-hover:opacity-100" />
            </div>
            <p className="text-[10px] text-slate-400 mt-1 leading-snug">
              Solicita dia e horário para bater um papo rápido.
            </p>
          </button>

          <button
            type="button"
            onClick={() => handleAddQuickTemplate('preco')}
            className="p-3 bg-slate-950/80 hover:bg-slate-900 border border-slate-800 hover:border-sky-500/40 rounded-xl text-left transition shadow-sm group"
          >
            <div className="text-xs font-bold text-sky-400 group-hover:text-sky-300 flex items-center justify-between">
              <span>💰 Preços & Orçamentos</span>
              <Plus className="w-3.5 h-3.5 opacity-60 group-hover:opacity-100" />
            </div>
            <p className="text-[10px] text-slate-400 mt-1 leading-snug">
              Informa que temos planos flexíveis e envia a proposta.
            </p>
          </button>
        </div>
      </div>
