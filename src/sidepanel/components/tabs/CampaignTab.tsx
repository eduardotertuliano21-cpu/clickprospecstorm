...
export const CampaignTab: React.FC<CampaignTabProps> = ({ selectedLeadIds, onClearSelectedLeads }) => {
  ...
  // Modos de Velocidade Amigáveis para Leigos
  const [speedMode, setSpeedMode] = useState<'safe' | 'balanced' | 'fast'>('safe');
  const [showAdvancedTiming, setShowAdvancedTiming] = useState(false);

  const handleSelectSpeed = (mode: 'safe' | 'balanced' | 'fast') => {
    setSpeedMode(mode);
    if (mode === 'safe') {
      setDelayMin(30);
      setDelayMax(70);
      setBreakAfterCount(10);
      setBreakDurationMin(5);
    } else if (mode === 'balanced') {
      setDelayMin(18);
      setDelayMax(40);
      setBreakAfterCount(15);
      setBreakDurationMin(3);
    } else {
      setDelayMin(10);
      setDelayMax(25);
      setBreakAfterCount(20);
      setBreakDurationMin(2);
    }
  };
        ...
        <div className="lg:col-span-7 space-y-4">
          <div className="bg-slate-900/60 p-4 rounded-xl border border-slate-800 space-y-3.5 shadow-lg">
              ...
              <div>
                <div className="flex flex-wrap items-center justify-between gap-1 mb-1.5">
                  ...
                  <div className="flex flex-wrap items-center gap-1.5">
                    <span className="text-[10px] text-slate-400 mr-1">Inserir dados do cliente:</span>
                    <button
                      type="button"
                      disabled={isRunning}
                      onClick={() => insertVariable('decisor')}
                      className="px-2 py-0.5 bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-500/40 text-emerald-300 font-bold rounded-lg text-[11px] transition shadow-sm active:scale-95"
                      title="Insere o nome do cliente automaticamente em cada mensagem"
                    >
                      + Nome do Cliente
                    </button>
                    <button
                      ...
                      className="px-2 py-0.5 bg-teal-950/80 hover:bg-teal-900 border border-teal-500/40 text-teal-300 font-bold rounded-lg text-[11px] transition shadow-sm active:scale-95"
                      title="Insere o nome da empresa do cliente"
                    >
                      + Empresa
                    </button>
                    <button
                      ...
                      className="px-2 py-0.5 bg-sky-950/80 hover:bg-sky-900 border border-sky-500/40 text-sky-300 font-bold rounded-lg text-[11px] transition shadow-sm active:scale-95"
                      title="Insere a cidade onde o cliente atua"
                    >
                      + Cidade
                    </button>
                  </div>
                ...
                <textarea
                  ...
                  placeholder="Escreva sua mensagem aqui... Use os botões acima para personalizar com o nome de cada pessoa."
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl p-3 text-xs text-slate-100 focus:outline-none focus:border-emerald-500 leading-relaxed font-sans"
                />
              </div>
            )}

            {/* SEGURANÇA E VELOCIDADE DE ENVIO (SIMPLES PARA LEIGOS) */}
            <div className="space-y-3 border-t border-slate-800/80 pt-3">
              <div className="flex items-center justify-between">
                <div className="text-xs font-bold text-emerald-400 flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" />
                  <span>Velocidade de Envio & Proteção Anti-Bloqueio</span>
                </div>
              ...
              {/* CARTÕES DE VELOCIDADE PRONTOS */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5">
                <div
                  onClick={() => !isRunning && handleSelectSpeed('safe')}
                  className={`p-2.5 rounded-xl border cursor-pointer transition text-left flex flex-col justify-between ${
                    speedMode === 'safe'
                      ? 'bg-emerald-950/60 border-emerald-500 text-emerald-300 shadow-md shadow-emerald-950/50'
                      : 'bg-slate-950 border-slate-800/90 text-slate-400 hover:border-slate-700 hover:text-slate-300'
                  } ${isRunning ? 'opacity-50 cursor-not-allowed' : ''}`}
                >
                  <div className="flex items-center gap-1.5 font-bold text-xs text-emerald-400">
                    <ShieldCheck className="w-3.5 h-3.5 shrink-0" />
                    <span>🛡️ Mais Seguro</span>
                  </div>
                  <p className="text-[10px] text-slate-400 mt-1 leading-snug">
                    30s a 70s. Recomendado para proteger seu WhatsApp.
                  </p>
                </div>

                <div
                  onClick={() => !isRunning && handleSelectSpeed('balanced')}
                  className={`p-2.5 rounded-xl border cursor-pointer transition text-left flex flex-col justify-between ${
                    speedMode === 'balanced'
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-400 mt-1 leading-snug">
                    18s a 40s. Ritmo constante para listas médias.
                  </p>
                </div>

                <div
                  onClick={() => !isRunning && handleSelectSpeed('fast')}
                  className={`p-2.5 rounded-xl border cursor-pointer transition text-left flex flex-col justify-between ${
                    speedMode === 'fast'
                ...
                >
                  ...
                  <p className="text-[10px] text-slate-400 mt-1 leading-snug">
                    10s a 25s. Para listas pequenas com maior urgência.
                  </p>
                </div>
              </div>

              {/* OPÇÕES DE VARIAÇÃO DE TEXTO */}
              <div className="space-y-1.5 pt-1">
                <label className="flex items-center gap-2 cursor-pointer text-xs text-slate-300 hover:text-slate-100">
                  ...
                  <span className="flex items-center gap-1 font-medium">
                    ...
                    Variar texto automaticamente com IA (Faz com que cada mensagem seja diferente para não parecer robô)
                  </span>
                </label>
                ...
                <label className="flex items-center gap-2 cursor-pointer text-xs text-slate-300 hover:text-slate-100">
                  <input
                    ...
                    className="rounded border-slate-700 text-emerald-500 focus:ring-0 w-3.5 h-3.5"
                  />
                  <span>Permitir variações de saudações (Ex: Olá, Oi, Tudo bem)</span>
                </label>
              </div>

              {/* ACCORDION DISCRETO PARA AJUSTE AVANÇADO DE TEMPO */}
              <div className="pt-1">
                <button
                  type="button"
                ...
                >
                  {showAdvancedTiming ? <ChevronUp className="w-3 h-3" /> : <ChevronDown className="w-3 h-3" />}
                  <span>⚙️ Personalizar tempos em segundos manualmente (Avançado)</span>
                </button>

                {showAdvancedTiming && (
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-2 bg-slate-950/80 p-3 rounded-xl border border-slate-800 animate-fadeIn">
                    <div>
                      <label className="text-[10px] text-slate-400 block mb-0.5">Espera Mínima (seg)</label>
                      <input
                        type="number"
                        disabled={isRunning}
                        value={delayMin}
                    ...
                    <div>
                      <label className="text-[10px] text-slate-400 block mb-0.5">Espera Máxima (seg)</label>
                      <input
                        type="number"
                        disabled={isRunning}
                        value={delayMax}
                    ...
                    <div>
                      <label className="text-[10px] text-slate-400 block mb-0.5">Pausar a cada (envios)</label>
                      <input
                        type="number"
                        disabled={isRunning}
            ...
            <div className="flex flex-col sm:flex-row gap-2.5 pt-3">
              <Tooltip text="Veja um exemplo de como a mensagem ficará antes de iniciar os envios">
                <button
                  type="button"
                  onClick={handlePreview}
                  disabled={isPreviewing || isRunning}
                  className="py-3 px-4 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-bold flex items-center justify-center gap-2 transition-all border border-slate-700 active:scale-95 sm:w-1/3"
                >
                  <Wand2 className="w-4 h-4 text-emerald-400 shrink-0" />
                  <span>{isPreviewing ? 'Gerando...' : '👁️ Ver Prévia'}</span>
                </button>
              </Tooltip>

              <Tooltip text={checkedLeadIds.length > 0 ? `Inicia os envios automáticos para os ${checkedLeadIds.length} contatos selecionados` : 'Marque os contatos na coluna da direita para iniciar'}>
                <button
                  type="button"
                  onClick={handleStartCampaign}
                  disabled={isRunning || checkedLeadIds.length === 0}
                  className={`flex-1 py-3 px-4 rounded-xl text-xs font-extrabold flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 ${
                    checkedLeadIds.length > 0
                      ? 'bg-gradient-to-r from-emerald-600 via-emerald-500 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white shadow-emerald-600/30 cursor-pointer'
                ...
                >
                  ...
                  <span>
                    {checkedLeadIds.length > 0 
                      ? `🚀 Iniciar Envio Agora (${checkedLeadIds.length} contatos)` 
                      : 'Marque contatos ao lado para enviar'}
                  </span>
                </button>
              </Tooltip>
            </div>
